...
//	wifiChannel.SetPropagationDelay("ns3::RandomPropagationDelayModel","Variable",RandomVariableValue (UniformVariable(0,0.000011)));
...
//	Ptr<NetDevice> senderIf = sourceNetDev.Get (0);
	...
	//bounds<<deltaXmin<<"|"<<deltaXmax<<"|"<<deltaYmin<<"|"<<deltaYmax;
	...
	for (int g = 0; g < groupModel; g++){
		...
		for (int k = 0; k < groupPoints; k++){
			double x, y, z;
			x = y = z = 0;
			x = UniformVariable(deltaXmin,deltaXmax).GetValue();
			y = UniformVariable(deltaYmin,deltaYmax).GetValue();
			positionAllocG[g]->Add(Vector(x, y, z));
			bounds<<"("<<x<<","<<y<<","<<z<<") ";
		}
//		Ptr<PositionAllocator> rec2 = positionAllocG[g]->GetObject<PositionAllocator> ();
		NS_LOG_DEBUG (bounds.str());
		bounds.str("");
	}
	...
	for (int g = 0; g < groupModel ; g++) {
		...
		mobilityG.SetPositionAllocator("ns3::RandomBoxPositionAllocator",
		...
		pause = 40.0+g;
		speed = 1.4;
		NS_LOG_DEBUG ("Group "<< g << " Pause "<< pause <<"s, Speed "<< speed << "ms");
		mobilityG.SetMobilityModel ("ns3::RandomWaypointMobilityModel",
		                             "Pause", RandomVariableValue (ConstantVariable(pause)),
