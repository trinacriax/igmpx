...
//	LogComponentEnable ("SimpleGlobalRoutingExample", LOG_LEVEL_INFO);
	LogComponentEnable ("IgmpxTestPim", LogLevel(LOG_INFO | LOG_PREFIX_TIME | LOG_PREFIX_NODE| LOG_PREFIX_FUNC));
...
//	LogComponentEnable ("DefaultSimulatorImpl", LogLevel(LOG_LEVEL_ALL | LOG_PREFIX_TIME | LOG_PREFIX_NODE| LOG_PREFIX_FUNC));
	...
	NS_LOG_DEBUG("Seed " << seed << " run "<< run << " sizeRouter "<< sizeRouter << " sizeClient " << sizeClient <<
			" sizeSource " << sizeSource << " routing " << routing << " range " << range << " cols " << cols <<
...
//	wifi.EnableLogComponents();
	...
	for (uint32_t n = 0;  n < routers.GetN() ; n++){
		 ...
		 std::cout << "Populate node \n";
		 Ptr<Ipv4L3Protocol> ip = routers.Get(n)->GetObject<Ipv4L3Protocol> ();
		 NS_ASSERT (ip !=0);
		 ...
		 for(ObjectVectorValue::Iterator j = interfaces.Begin (); j != interfaces.End (); j++)
		   {
			 ...
			 std::cout << "Detected address " << hwAddr << std::endl;
